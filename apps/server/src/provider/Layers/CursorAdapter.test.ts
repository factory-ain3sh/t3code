...
cursorAdapterTestLayer("CursorAdapterLive", (it) => {
  ...
  it.effect("queues the Cursor prompt before an immediate interrupt", () =>
    Effect.gen(function* () {
      const adapter = yield* CursorAdapter;
      const settings = yield* ServerSettingsService;
      const threadId = ThreadId.make("cursor-prompt-before-interrupt");
      const tempDir = yield* Effect.promise(() =>
        NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "cursor-acp-ordering-")),
      );
  ...
  it.effect("queues the Cursor prompt before an immediate stop", () =>
    Effect.gen(function* () {
      const adapter = yield* CursorAdapter;
      const settings = yield* ServerSettingsService;
      const threadId = ThreadId.make("cursor-prompt-before-stop");
      const tempDir = yield* Effect.promise(() =>
        NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "cursor-acp-ordering-")),
      );
      const requestLogPath = NodePath.join(tempDir, "requests.ndjson");
  ...
  it.effect("queues the Cursor prompt before replacing its session", () =>
    Effect.gen(function* () {
      const adapter = yield* CursorAdapter;
      const settings = yield* ServerSettingsService;
      const threadId = ThreadId.make("cursor-prompt-before-replace");
      const tempDir = yield* Effect.promise(() =>
        NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "cursor-acp-ordering-")),
      );
      const requestLogPath = NodePath.join(tempDir, "requests.ndjson");
  ...
  it.effect("drains Cursor events before settling a failed prompt", () =>
    Effect.gen(function* () {
      const adapter = yield* CursorAdapter;
      const settings = yield* ServerSettingsService;
      const threadId = ThreadId.make("cursor-prompt-failure-drain-order");
      const wrapperPath = yield* Effect.promise(() =>
        makeMockAgentWrapper({

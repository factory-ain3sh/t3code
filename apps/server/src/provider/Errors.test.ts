...
describe("ProviderDriverError", () => {
  it("keeps the wrapper detail stable while rendering the structured cause", () => {
    const error = new ProviderDriverError({
      driver: "droid",
      instanceId: "droid-default",
      detail: "Failed to build Droid snapshot.",
      cause: new Error("Droid executable was not found"),
    });

    expect(error.message).toBe(
      "Provider driver 'droid' failed to create instance 'droid-default': Failed to build Droid snapshot. Cause: Droid executable was not found",
    );
  });
